...
export function createBackgroundPanelController({ elements, getSceneSession, saveState, refreshClipSettings }) {
  ...
  function handlePointerMove(event) {
    ...
    const nextValue = clampBackgroundNumber(
      numberDrag.startValue + stepCount * numberDrag.step,
      numberDrag.min,
      numberDrag.max
    );
    numberDrag.input.value = formatBackgroundInputValue(nextValue, numberDrag.step);

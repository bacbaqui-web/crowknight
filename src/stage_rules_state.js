...
export function createDefaultStageRules() {
  return {
    ...
    worldPhysics: createDefaultWorldPhysicsRules(),
...
export function normalizeStageRules(saved) {
  return {
    ...
    worldPhysics: normalizeWorldPhysicsRules(saved?.worldPhysics),
...
export function createDefaultWorldPhysicsRules() {
  return {
    gravity: 1,
    inertia: 30,
    airControl: 0,
...
export function normalizeWorldPhysicsRules(saved) {
  const defaults = createDefaultWorldPhysicsRules();
  return {
...
export function createDefaultEnemyActorRule() {
  return {
    hitCancelChance: 0,
...
export function normalizeEnemyActorRule(saved) {
  const defaults = createDefaultEnemyActorRule();
  return {
    hitCancelChance: clampNumber(saved?.hitCancelChance, 0, 100, defaults.hitCancelChance),
    hitCancelFlashFrames: clampNumber(saved?.hitCancelFlashFrames, 1, 120, defaults.hitCancelFlashFrames),
  };
}

function normalizeEnemyActorRulesByActor(saved, fallback = {}) {
  const source = saved && typeof saved === 'object' && !Array.isArray(saved) ? saved : fallback;
  return Object.fromEntries(
    Object.entries(source || {})
      .map(([actorId, rule]) => {
        const id = nonEmptyString(actorId);
        if (!id) return null;
        return [id, normalizeEnemyActorRule(rule)];
      })
      .filter(Boolean)
  );
}
...
function defaultEnemyActorIds() {
  return ACTOR_DEFS.filter((actor) => !isPlayerCharacter(actor)).map((actor) => actor.id);
}
